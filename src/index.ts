...
export async function mmac({
    ...
    const oldContent = await fs.readFile(filepath).then((b: Buffer) => b.toString()).catch(
        (e: Error) => {
    ...
    if (mark.startIndex === -1) {
        throw new Error(`Could not find start content comment in file "${filepath}"`);
    }
    ...
    if (mark.startIndex > mark.endIndex) {
        throw new Error(`End comment is located before content start comment in file "${filepath}"`);
    }
